...
% };
run_specs = {                                    
    {'SC011_021919', '0', '0,0', '0', {'cortex' 'thalamus' 'default'}}
};
...
for curr = run_specs
    ...
    if runTPrime
        ...
        session_id = strcat(spec{1}, '_TPrime');
        input_json = fullfile(json_directory, strcat(session_id, '-input.json'));
        output_json = fullfile(json_directory, strcat(session_id, '-output.json'));
        ...
        for i=1:length(prob_list)
            prb = string(prob_list{i});
            sync_extract = strcat('-SY=', prb, ',-1,6,500');
            im_ex_list = strcat(im_ex_list, ' ', sync_extract);
        end

        disp(strcat('im_ex_list: ', im_ex_list));
        ...
        command = strcat(PYENV_PATH, " -W ignore -m ecephys_spike_sorting.modules.", "tPrime_helper", " --input_json ", input_json, " --output_json ", output_json);
        disp(command);
        system(command);
    end
